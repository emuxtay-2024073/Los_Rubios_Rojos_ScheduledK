...
class AppointmentService {
    ...
    static parseAppointmentTime(date, timeValue, fieldName) {
        ...
        if (timeValue.includes("T")) {
            ...
            if (Number.isNaN(parsed.getTime())) {
                throw new Error(`Formato de ${fieldName} inválido. Use HH:mm, HH:mm:ss o ISO datetime.`);
            }
        ...
        if (timeParts.length < 2 || timeParts.length > 3) {
            throw new Error(`Formato de ${fieldName} inválido. Use HH:mm o HH:mm:ss.`);
        }
    ...
    static async createAppointment(data) {
        ...
        if (overlapping) {
            throw new Error("El coordinador ya tiene una cita programada en ese horario");
        }
    ...
    static async rescheduleAppointment(id, newDate, newStartTime, newEndTime, performedBy) {
        const appointment = await Appointment.findById(id);
        if (!appointment) throw new Error("La cita no existe en el sistema");
        ...
        if (newAppointmentDate <= originalDate) {
            throw new Error("No se puede reagendar para el mismo día ni días anteriores a la cita cancelada");
        }
        ...
        if (overlapping) {
            throw new Error("El coordinador ya tiene una cita programada en ese nuevo horario");
        }
